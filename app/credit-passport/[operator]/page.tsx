...
export default function CreditPassportPage() {
  ...
  const hasValidOperator = isAddress(operatorParam)
      ...
      <main style={{ maxWidth: 1280, margin: "0 auto", padding: "56px 32px 72px" }}>
        <section style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 24, marginBottom: 28 }}>
          <div>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", marginTop: 16 }}>
              <span style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: "var(--text-primary)", padding: "6px 10px", border: "1px solid var(--border)", borderRadius: 8 }}>{hasValidOperator ? shortAddress(operator) : "Invalid operator address"}</span>
        ...
        {!hasValidOperator ? (
          <div className="card" style={{ padding: 32 }}>
            <p style={{ color: "var(--danger)", fontWeight: 600, marginBottom: 8 }}>Invalid operator address.</p>
            <p style={{ color: "var(--text-muted)", fontSize: 14 }}>Credit Passport routes require an EVM address.</p>
          </div>
        ) : (
          <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 24px" }}>
                {controls.map((control) => (
                  <div key={control.label} style={{ display: "flex", gap: 12, padding: "14px 0", borderBottom: "1px solid var(--border)" }}>
                    <span style={{ color: control.status.startsWith("Not") ? "var(--text-dim)" : "var(--emerald)", fontSize: 16, lineHeight: 1.4 }}>{control.status.startsWith("Not") ? "-" : "✓"}</span>
                    <div>
