...
export default function AdminPage() {
  ...
  const showAdvancedLiquidityControls = process.env.NEXT_PUBLIC_ENABLE_ADVANCED_LIQUIDITY_CONTROLS === "true"
  const hasYieldOracle = CONTRACTS.yieldOracle !== "0x0000000000000000000000000000000000000000"
  ...
  const { data: liquidityManagerData, refetch: refetchLiquidityManager } = useReadContracts({
    ...
    query: { enabled: isOwner && showAdvancedLiquidityControls && hasLiquidityManager, refetchInterval: 15000 },
  })
  const { data: liquidityVaultData, refetch: refetchLiquidityVault } = useReadContracts({
    ...
    query: { enabled: isOwner && showAdvancedLiquidityControls && hasLiquidityVault, refetchInterval: 15000 },
  })
  ...
  const { data: liquidityData } = useReadContracts({
    ...
    query: { enabled: showAdvancedLiquidityControls && !!liquidityCycle, refetchInterval: 10000 },
  })
  ...
  const { data: liquidityBalances } = useReadContracts({
    ...
    query: { enabled: showAdvancedLiquidityControls && !!address && !!liquidityToken, refetchInterval: 10000 },
  })
  const { data: launchData, refetch: refetchLaunch } = useReadContracts({
    ...
    query: { enabled: isOwner && showAdvancedLiquidityControls && hasLiquidityManager && !!liquidityCycle, refetchInterval: 10000 },
  })
  ...
  async function handleSetOracle() {
    ...
    try {
      ...
      showToast("Estimate override saved")
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "48px 32px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 28 }}>
          {[
            ...
            { label: "Reserve pool", value: `$${reserveBal.toLocaleString(undefined, { maximumFractionDigits: 2 })}`, color: "var(--gold)" },
          ].map((stat) => (
        ...
        <div className="card" style={{ padding: 18, marginBottom: 20 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14 }}>
            {[
              ...
              ["Operator approval mode", ["Manual", "Open", "Collateral"][currentMode] ?? "Unknown", "Controls how SME operators enter the protocol."],
            ].map(([label, value, sub]) => (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          ...
          <div className="card" style={{ padding: 24 }}>
            <h3 style={{ fontSize: 15, fontWeight: 600, marginBottom: 6 }}>Cycle revenue/risk estimate override</h3>
            <p style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 18, lineHeight: 1.7 }}>Optional admin override for a cycle&apos;s expected revenue, estimated cost, and risk score. Normally this is set during cycle creation.</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div><label style={{ fontSize: 12, color: "var(--text-muted)", display: "block", marginBottom: 5 }}>Cycle contract address</label><input type="text" placeholder="0x..." value={oracleForm.cycle} disabled={!hasYieldOracle} onChange={(e) => setOracleForm((form) => ({ ...form, cycle: e.target.value }))} /></div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div><label style={{ fontSize: 12, color: "var(--text-muted)", display: "block", marginBottom: 5 }}>Expected revenue (USDC)</label><input type="number" placeholder="61000" value={oracleForm.revenue} disabled={!hasYieldOracle} onChange={(e) => setOracleForm((form) => ({ ...form, revenue: e.target.value }))} /></div>
                <div><label style={{ fontSize: 12, color: "var(--text-muted)", display: "block", marginBottom: 5 }}>Estimated cost (USDC)</label><input type="number" placeholder="50000" value={oracleForm.cost} disabled={!hasYieldOracle} onChange={(e) => setOracleForm((form) => ({ ...form, cost: e.target.value }))} /></div>
              </div>
              <div>
                <label style={{ fontSize: 12, color: "var(--text-muted)", display: "block", marginBottom: 5 }}>Risk score (1-10)</label>
                <input type="range" min="1" max="10" value={oracleForm.risk} disabled={!hasYieldOracle} onChange={(e) => setOracleForm((form) => ({ ...form, risk: e.target.value }))} style={{ padding: 0, height: "auto", background: "transparent", border: "none" }} />
              ...
              {!hasYieldOracle && <p style={{ fontSize: 12, color: "var(--warning)", lineHeight: 1.6 }}>Yield oracle is not configured for this deployment.</p>}
              <button className="btn-primary" onClick={handleSetOracle} disabled={!hasYieldOracle || !oracleForm.cycle || !oracleForm.revenue || !oracleForm.cost}>Save estimate override</button>
            </div>
          </div>
          ...
          <div className="card" style={{ padding: 24, gridColumn: "1 / -1" }}>
            ...
            {!hasLiquidityManager || !hasLiquidityVault ? (
              <div style={{ padding: 16, background: "rgba(224,82,82,0.06)", border: "1px solid rgba(224,82,82,0.2)", borderRadius: 10 }}>
                <p style={{ fontSize: 13, color: "var(--danger)" }}>Configure liquidity vault and manager addresses through environment variables before enabling advanced liquidity controls.</p>
              </div>
            ) : (
